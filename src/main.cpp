		...
		for (int index = 0; index < ready; ++index)
		{
			...
			if (ready_fd == server_fd)
			{
				...
				if (n > 0)
				{
					...
					// Reading and parsing the request should be done by some function that saves its state.
					// then if that is done the response will be generated by another function.
					// if (!not_done)
					// {
					//        // 1. read into the request.
					//        // 2. parse what was read.
					//        // 3. check if everything was parsed from the client.. 
					//		  // 4. parsing responsibilty:
					//				- headers
					//				- body
					// } else { the requested was fully parsed now generate the response using the request
					//		  response = generate(request)
					//		  // 1. write the response.
					//		  // 2. close and cleanup the client
					// }
