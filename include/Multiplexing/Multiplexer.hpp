...
public:
	...
	static Client   *register_client(uint32_t e, Server *server) throw(const char *);
	...
	static ssize_t read(int fd, void *buf, size_t size) throw(const char *);
	static ssize_t write(int fd, const void *buf, size_t size) throw(const char *);
