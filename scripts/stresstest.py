...
CONCURRENT_TASKS = 20                 # Number of concurrent workers running simultaneously
TOTAL_REQUESTS_PER_WORKER = 200        # Number of cycles (Upload + Delete) each worker performs
...
async def main():
    ...
    total_ops = stats["upload_success"] + stats["upload_fail"] + stats["delete_success"] + stats["delete_fail"] + stats["get_success"] + stats['delete_fail']
    ...
    print(f"🟢 Get Successes:     {stats['get_success']}")
    print(f"🔴 Get Failures:      {stats['get_fail']}")
    ...
    if stats["upload_fail"] > 0 or stats["delete_fail"] > 0 or stats["exceptions"] > 0 or stats["get_fail"] > 0:
